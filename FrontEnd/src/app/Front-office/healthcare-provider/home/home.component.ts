...
export class HomeComponent implements OnInit {
  ...
  private updateProviderStats(histories: MedicalHistoryResponse[] | any, alerts: AlertResponse[]): void {
    // Ensure histories is an array
    if (!Array.isArray(histories)) {
      if (histories?.content && Array.isArray(histories.content)) {
    ...
    const severeCases = histories.filter((history: MedicalHistoryResponse) => (history.progressionStage || '').toUpperCase() === 'SEVERE').length;
